...
class GANerAidGenerator(nn.Module):
    ...
    def init_hidden(self, batch_size):
        # function to init the hidden layers — use self.device consistently
        return (torch.randn(self.lstm_layers * self.directions, batch_size, self.hidden_size, device=self.device),
                torch.randn(self.lstm_layers * self.directions, batch_size, self.hidden_size, device=self.device))
    ...
    def forward(self, x):
        ...
        output = torch.zeros(x.shape[0], self.rows, self.columns, device=self.device)
        c = 0
