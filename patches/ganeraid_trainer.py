...
def real_data_target(batch_size, device):
    data = Variable(torch.ones(batch_size, 1, device=device))
    return data


def fake_data_target(batch_size, device):
    data = Variable(torch.zeros(batch_size, 1, device=device))
    return data
...
class GanTrainer:
    ...
    def train_on_batch(self, gan, real_batch, d_optimizer, g_optimizer):
        ...
        # Generate fake data
        fake_data = gan.generator(noise(real_data.size(0), gan.generator.noise_size, device=device)).detach()
        # Train D
        # Reset gradients
        ...
        error_real = self.loss(prediction_real, real_data_target(real_data.size(0), device))
        error_real.backward()

        # 1.2 Train on Fake Data
        prediction_fake = gan.discriminator(fake_data)
        # Calculate error and backpropagate
        error_fake = self.loss(prediction_fake, fake_data_target(real_data.size(0), device))
        error_fake.backward()
        ...
        # 2. Train Generator
        # Generate fake data
        fake_data = gan.generator(noise(real_batch.size(0), gan.generator.noise_size, device=device))
        # Train G
        # Reset gradients
        ...
        # Calculate error and backpropagate
        g_error = self.loss(prediction, real_data_target(prediction.size(0), device))
        g_error.backward()
