...
def get_n_trials(config: Dict[str, Any]) -> int:
    """
    Get the pilot trial count from config.

    Args:
        config: Notebook configuration dictionary

    Returns:
        Number of trials (n_trials_pilot)
    """
    return config.get("n_trials_pilot", 5)
