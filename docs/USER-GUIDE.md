...
| `sdmetrics_evaluation_summary.csv` + `sdmetrics_metric_catalog.csv` + `sdmetrics_radar_chart.png` + `sdmetrics_heatmap.png` | Independent third-party scoring from the SDMetrics library (`sdv.dev`). **Source: SDMetrics — does not feed SDAC composite scoring.** Columns marked `†` overlap with SDAC. | See the Section 5 entry for full details — same files are produced in §3 as a baseline cross-check. |
...
| `sdmetrics_evaluation_summary.csv` | Independent third-party scoring from the public SDMetrics library (`sdv.dev`). One row per model; ~25 metrics across Coverage, Validity, Shapes, Pair-Trends, Detection, Privacy, ML-Efficacy, Aggregate. **Source: SDMetrics — does not feed SDAC composite scoring.** | A cross-check, not a replacement. Where a column overlaps with SDAC (e.g., `Shape_KSComplement` ≈ `Fidelity_KS`), values should agree directionally; large divergences are worth investigating. The two `Quality_Overall` and `Diagnostic_Overall` columns give you SDV's packaged 0-1 aggregate scores for a quick orientation. |
| `sdmetrics_metric_catalog.csv` | One row per SDMetrics metric: `metric`, `category`, `sdac_overlap`, `sdac_counterpart`. | Lookup table for reading the SDMetrics summary. The `†` markers in the SDMetrics printed table and heatmap correspond to `sdac_overlap=True` rows here. |
| `sdmetrics_radar_chart.png` | SDMetrics composite scores per category (Coverage, Validity, Shapes, …). Captioned `Source: SDMetrics (sdv.dev) — independent evaluation`. | Same shape as the SDAC radar but on SDMetrics axes. Useful for spotting categories the SDAC composite doesn't directly cover (Coverage and Validity in particular). |
| `sdmetrics_heatmap.png` | Models × SDMetrics metrics. Columns suffixed with `†` overlap with SDAC. Captioned `Source: SDMetrics (sdv.dev). † columns also exist in SDAC suite (different implementations).` | Use the `†` markers to do the SDAC-vs-SDMetrics sanity check at a glance. |
