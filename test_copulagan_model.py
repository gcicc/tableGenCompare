#!/usr/bin/env python3
"""
Test script for CopulaGAN model implementation.
Validates CopulaGAN integration with the framework.
"""
...
def create_mixed_data():
    ...
    n_samples = 800  # Good size for CopulaGAN
    data = pd.DataFrame({
...
def test_copulagan_availability():
    """Test if CopulaGAN (SDV) is available."""
    print("Testing CopulaGAN availability...")
    ...
    try:
        ...
        if available_models.get('copulagan', False):
            print("✅ CopulaGAN is available")
            return True
        else:
            print("⏭️ CopulaGAN not available - dependencies not installed")
            print("To install: pip install sdv")
            return False
            
    except Exception as e:
        print(f"❌ Error checking CopulaGAN availability: {e}")
        return False

def test_copulagan_model():
    """Test CopulaGAN model training and generation."""
    print("Testing CopulaGAN model...")
    
    try:
        from models.model_factory import ModelFactory
        
        # Create CopulaGAN model
        model = ModelFactory.create('copulagan', device='cpu', random_state=42)
        print(f"✅ CopulaGAN model created: {model.__class__.__name__}")
        ...
        print("Training CopulaGAN model (this may take 3-5 minutes)...")
        ...
        categorical_cols = ['education', 'employment_type', 'region']
        for col in categorical_cols:
            if col in synthetic_data.columns:
                ...
                if synthetic_values.issubset(original_values):
                    ...
                    print(f"  ⚠️ {col} has extra values: {extra_values}")
        ...
        print(f"⏭️ CopulaGAN dependencies not available: {e}")
        return True  # Not a failure, just not available
    except Exception as e:
        print(f"❌ CopulaGAN model test failed: {e}")
        import traceback
        traceback.print_exc()
        return False

def test_copulagan_evaluation_integration():
    """Test CopulaGAN integration with evaluation framework."""
    print("Testing CopulaGAN-Evaluation integration...")
    ...
    try:
        ...
        print("Training CopulaGAN for integration test...")
        model.train(original_data, epochs=50, verbose=False)  # Quick training for testing
        ...
        dataset_metadata = {
            'dataset_info': {'name': 'copulagan_integration_test', 'description': 'CopulaGAN integration test'},
        ...
        results = evaluator.run_complete_evaluation(
            ...
            output_dir="test_copulagan_output",
            target_column='target'
        )
        
        print("✅ CopulaGAN-Evaluation integration successful!")
        ...
        # Check CopulaGAN-specific training metadata in results
...
def test_copulagan_hyperparameter_optimization():
    """Test CopulaGAN with hyperparameter optimization."""
    print("Testing CopulaGAN hyperparameter optimization...")
    ...
    try:
        ...
        configs = [
            {'epochs': 50, 'batch_size': 100, 'generator_lr': 0.0001, 'discriminator_lr': 0.0001},
            {'epochs': 75, 'batch_size': 200, 'generator_lr': 0.0002, 'discriminator_lr': 0.0002},
            {'epochs': 60, 'batch_size': 150, 'generator_lr': 0.0005, 'discriminator_lr': 0.0003}
        ]
        ...
        for i, config in enumerate(configs):
            ...
            eval_results = evaluator.run_complete_evaluation(
                ...
                synthetic_data=synthetic_data,
                dataset_metadata={
                    'dataset_info': {'name': f'copulagan_config_test_{i}'},
                    'target_info': {'column': 'target', 'type': 'binary'}
                },
                output_dir=f"test_copulagan_config_{i}",
                target_column='target'
            )
        ...
        print(f"\n✅ CopulaGAN hyperparameter optimization completed!")
...
def test_copulagan_vs_other_models():
    """Quick comparison test between CopulaGAN and other available models."""
    print("Testing CopulaGAN vs other models comparison...")
    ...
    try:
        ...
        if not available_models.get('copulagan', False):
        ...
        for model_name in comparison_models:
            ...
            if model_name == 'copulagan':
                model.train(test_data, epochs=50, batch_size=100, verbose=False)
        ...
        # Compare results
        print(f"\n{'='*50}")
        print("CopulaGAN vs Other Models Comparison:")
        print(f"{'='*50}")
        ...
        print("\n✅ CopulaGAN comparison completed")
        return True
...
def run_comprehensive_copulagan_test():
    """Run comprehensive CopulaGAN test suite."""
    print("=" * 60)
    print("COPULAGAN MODEL TEST SUITE")
    print("=" * 60)
    
    # Test 1: CopulaGAN availability
    available = test_copulagan_availability()
    if not available:
        print("\n[INFO] CopulaGAN not available. To install:")
        print("pip install sdv")
        print("\nSkipping CopulaGAN tests.")
        return False
    
    results = {}
    
    # Test 2: CopulaGAN model
    print("\n" + "=" * 60)
    results['model'] = test_copulagan_model()
    
    # Test 3: Integration with evaluation framework
    print("\n" + "=" * 60)
    results['integration'] = test_copulagan_evaluation_integration()
    
    # Test 4: Hyperparameter optimization
    print("\n" + "=" * 60)
    results['hyperparameters'] = test_copulagan_hyperparameter_optimization()
    
    # Test 5: Comparison with other models
    print("\n" + "=" * 60)
    results['comparison'] = test_copulagan_vs_other_models()
    
    # Summary
    print("\n" + "=" * 60)
    print("COPULAGAN TEST RESULTS")
    print("=" * 60)
    ...
    if passed_tests == total_tests:
        print("🎉 All CopulaGAN tests passed!")
        print("✅ CopulaGAN is ready for production use alongside other models.")
        print("\nCopulaGAN Features:")
        print("  - Copula-based statistical modeling")
        print("  - Excellent marginal distribution preservation")
        print("  - Strong handling of mixed data types")
        print("  - Robust dependency modeling")
        return True
    else:
        print("⚠️ Some CopulaGAN tests failed or were skipped.")
        return False

if __name__ == "__main__":
    success = run_comprehensive_copulagan_test()
